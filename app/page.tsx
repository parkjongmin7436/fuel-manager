...
export default function Home() {
  ...
  const [monthlyBudget, setMonthlyBudget] = useState(0) // ✅ 초기값 0원으로 변경
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setMonthlyBudget(budgetData?.value ? parseInt(budgetData.value) : 0) // ✅ 기본값 0
  ...
  const handleAddFuel = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        date: formData.date,
        time: currentTime, // ✅ 자동으로 현재 시간 저장
                      ...
                      <div className="bg-blue-50 border-2 border-blue-300 rounded-xl p-5 space-y-3">
                        <div>
                          ...
                          <input
                            ...
                            value={record.date}
                            onChange={(e) => {
                              const updated = fuelRecords.map(r => r.id === record.id ? { ...r, date: e.target.value } : r)
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={record.time || ''}
                            onChange={(e) => {
                              const updated = fuelRecords.map(r => r.id === record.id ? { ...r, time: e.target.value } : r)
                              setFuelRecords(updated)
                            }}
                            className="w-full px-3 py-2 border rounded-lg text-sm"
                          />
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            ...
                            <input
                              ...
                              value={record.region || ''}
                              onChange={(e) => {
                                const updated = fuelRecords.map(r => r.id === record.id ? { ...r, region: e.target.value } : r)
                                setFuelRecords(updated)
                              }}
                              className="w-full px-3 py-2 border rounded-lg text-sm"
                              maxLength={20}
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = fuelRecords.map(r => r.id === record.id ? { ...r, station: e.target.value } : r)
                                setFuelRecords(updated)
                              }}
                              className="w-full px-3 py-2 border rounded-lg text-sm"
                              maxLength={50}
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = fuelRecords.map(r => r.id === record.id ? { ...r, price_per_liter: parseInt(e.target.value) } : r)
                                setFuelRecords(updated)
                              }}
                              className="w-full px-3 py-2 border rounded-lg text-sm"
                              min="0"
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = fuelRecords.map(r => r.id === record.id ? { ...r, fuel_amount: parseFloat(e.target.value) } : r)
                                setFuelRecords(updated)
                              }}
                              className="w-full px-3 py-2 border rounded-lg text-sm"
                              min="0"
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = fuelRecords.map(r => r.id === record.id ? { ...r, distance: parseInt(e.target.value) } : r)
                                setFuelRecords(updated)
                              }}
                              className="w-full px-3 py-2 border rounded-lg text-sm"
                              min="0"
                          ...
                          {openMenuId === record.id && (
                            <div className="absolute right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-10 min-w-[100px]">
                              <button
                                ...
                                className="w-full text-left px-4 py-2 hover:bg-gray-100 text-sm font-semibold text-gray-700 rounded-t-lg"
                              >
                                수정
                              </button>
                              <button
                                onClick={() => handleDeleteFuel(record.id)}
                                className="w-full text-left px-4 py-2 hover:bg-red-50 text-sm font-semibold text-red-600 rounded-b-lg"
                              >
                                삭제
                              </button>
                            </div>
                    ...
                    <div className="bg-blue-50 border-2 border-blue-300 rounded-xl p-4 space-y-3">
                      <div>
                        ...
                        <input
                          ...
                          value={record.date}
                          onChange={(e) => {
                            const updated = tollRecords.map(r => r.id === record.id ? { ...r, date: e.target.value } : r)
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const updated = tollRecords.map(r => r.id === record.id ? { ...r, section: e.target.value } : r)
                            setTollRecords(updated)
                          }}
                          className="w-full px-3 py-2 border rounded-lg text-sm"
                          maxLength={50}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={record.amount}
                          onChange={(e) => {
                            const updated = tollRecords.map(r => r.id === record.id ? { ...r, amount: parseInt(e.target.value) } : r)
                            setTollRecords(updated)
                          }}
                          className="w-full px-3 py-2 border rounded-lg text-sm"
                          min="0"
